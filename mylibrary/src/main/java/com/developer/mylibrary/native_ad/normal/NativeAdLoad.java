...
public class NativeAdLoad {
    ...
    public static void loadSequenceNativeAd(Activity activity, RelativeLayout relativeLayout, LinearLayout linearLayout, String nativeAdSize, boolean isShowExtraNative) {
        if (AdsMasterClass.getAdsDataModel().getNative_show_sequence() != null && AdsMasterClass.getAdsDataModel().getNative_show_sequence().trim().length() > 0 && !AdsMasterClass.getAdsDataModel().getNative_show_sequence().equals("0")) {
            ...
            if (nextAd.equals(AllAdsType.g.name())) {
                ...
                loadGoogleNative(activity, relativeLayout, linearLayout, AdsMasterClass.getGoogleNativeId(activity, AdsMasterClass.getAdsDataModel().getAdx_native_id()), nextAd, nativeAdSize);
            } else if (nextAd.equals(AllAdsType.adx2.name())) {
                loadGoogleNative(activity, relativeLayout, linearLayout, AdsMasterClass.getGoogleNativeId(activity, AdsMasterClass.getAdsDataModel().getAdx2_native_id()), nextAd, nativeAdSize);
            } else if (nextAd.equals(AllAdsType.ab.name())) {
                loadGoogleNative(activity, relativeLayout, linearLayout, AdsMasterClass.getGoogleNativeId(activity, AdsMasterClass.getAdsDataModel().getAppbaroda_native_id()), nextAd, nativeAdSize);
            } else if (nextAd.equals(AllAdsType.f.name())) {
                loadFacebookNative(activity, relativeLayout, linearLayout, AdsMasterClass.getFacebookNativeId(activity, AdsMasterClass.getAdsDataModel().getFacebook_native_id()), nextAd, nativeAdSize);
            } else if (nextAd.equals(AllAdsType.a.name())) {
                loadApplovinNative(activity, relativeLayout, linearLayout, AdsMasterClass.getApplovinNativeId(activity, AdsMasterClass.getAdsDataModel().getApplovin_native_id()), nextAd, nativeAdSize);
    ...
    public static void loadGoogleNative(Activity activity, RelativeLayout relativeLayout, LinearLayout linearLayout, String adID, String currentAd, String nativeAdSize) {
        if (adID != null && adID.trim().length() > 0) {
            showLoadingAdsLayout(relativeLayout);
            AdLoader.Builder builder = new AdLoader.Builder(activity, adID);
            builder.forNativeAd(new NativeAd.OnNativeAdLoadedListener() {
                @Override
                public void onNativeAdLoaded(@NonNull NativeAd nativeAd) {
                    AdsMasterClass.showAdTag(AdsLogTag.NativeAdLoad.name(), "loadGoogleNative - loaded");
                    hideLoadingAdsLayout(relativeLayout);
    ...
    public static void loadFacebookNative(Activity activity, RelativeLayout relativeLayout, LinearLayout linearLayout, String adID, String currentAd, String nativeAdSize) {
        if (adID != null && adID.trim().length() > 0) {
            showLoadingAdsLayout(relativeLayout);
            ...
            NativeAdListener nativeAdListener = new NativeAdListener() {
                ...
                public void onError(Ad ad, com.facebook.ads.AdError adError) {
                    AdsMasterClass.showAdTag(AdsLogTag.NativeAdLoad.name(), "loadFacebookNative - failed " + adError.getErrorMessage());
                    NativeAdFailed.showNativeAdOnFailed(activity, relativeLayout, linearLayout, nativeAdSize, currentAd);
                }

                @Override
                public void onAdLoaded(Ad ad) {
                    AdsMasterClass.showAdTag(AdsLogTag.NativeAdLoad.name(), "loadFacebookNative - loaded");
        ...
        if (adID != null && adID.trim().length() > 0) {
            ...
            nativeAdLoader.setNativeAdListener(new MaxNativeAdListener() {
                ...
                public void onNativeAdLoaded(MaxNativeAdView maxNativeAdView, MaxAd maxAd) {
                    ...
                    AdsMasterClass.showAdTag(AdsLogTag.NativeAdLoad.name(), "loadApplovinNative - loaded");
                    hideLoadingAdsLayout(relativeLayout);
                    linearLayout.setVisibility(View.VISIBLE);
                    linearLayout.removeAllViews();
                ...
                public void onNativeAdLoadFailed(String s, MaxError maxError) {
                    ...
                    AdsMasterClass.showAdTag(AdsLogTag.NativeAdLoad.name(), "loadApplovinNative - failed " + maxError.getMessage());
                    NativeAdFailed.showNativeAdOnFailed(activity, relativeLayout, linearLayout, nativeAdSize, currentAd);
                }
            });
            if (nativeAdSize.equals(NativeAdSize.LARGE.name())) {
    ...
    public static void loadQurekaNative(Activity activity, RelativeLayout relativeLayout, LinearLayout linearLayout, String nativeAdSize) {
        AdsMasterClass.showAdTag(AdsLogTag.NativeAdLoad.name(), "loadQurekaNative - loaded");
        showLoadingAdsLayout(relativeLayout);
        hideLoadingAdsLayout(relativeLayout);
    ...
    public static void showLoadingAdsLayout(RelativeLayout relativeLayout) {
        if (relativeLayout != null) {
            relativeLayout.setVisibility(View.VISIBLE);
            if (AdsMasterClass.getAdsDataModel() != null && AdsMasterClass.getAdsDataModel().getNative_preload() == 2) {
    ...
    public static void hideLoadingAdsLayout(RelativeLayout relativeLayout) {
        if (relativeLayout != null) {
            if (relativeLayout.getChildCount() > 0) {
                relativeLayout.getChildAt(0).setVisibility(View.GONE);
            }
